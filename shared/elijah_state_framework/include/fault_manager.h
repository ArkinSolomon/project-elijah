...
private:
  ...
  std::map<uint8_t, FaultDefinition<FaultKeyType>> fault_map;

  void register_fault(CommunicationChannel communication_channel, std::string fault_name);
  FaultDefinition<FaultKeyType>* find_fault(FaultKeyType fault_key);
};
                                                ...
                                                CommunicationChannel communication_channel)
{
  fault_map.emplace(
    next_fault_bit,
    FaultDefinition<FaultKeyType>(key, fault_name, next_fault_bit, communication_channel, false)
  );
  next_fault_bit++;
}
...
{
  encoded_size = 0;
  for (const FaultDefinition<FaultKeyType>& entry : std::views::values(fault_map))
  {
...
template <EnumType FaultKeyEnumType>
                                                          ...
                                                          uint8_t& fault_bit, bool& did_fault_change)
{
  ...
  FaultDefinition<FaultKeyEnumType>* changing_fault_def = find_fault(key);
  ...
  if (changing_fault_def->get_communication_channel() != CommunicationChannel::None)
  {
    ...
    for (const FaultDefinition<FaultKeyEnumType>& other_com_fault_def : std::views::values(fault_map))
    {
    ...
    FaultDefinition<FaultKeyEnumType>& com_entry = fault_map.at(
...
bool FaultManager<FaultKeyType>::is_faulted(FaultKeyType key)
{
  ...
  FaultDefinition<FaultKeyType>* def = find_fault(key);
  assert(def != nullptr);
...
template <EnumType FaultKeyType>
FaultDefinition<FaultKeyType>* FaultManager<FaultKeyType>::find_fault(FaultKeyType fault_key)
{
  for (FaultDefinition<FaultKeyType>& fault_def : std::views::values(fault_map))
  {
    typename FaultDefinition<FaultKeyType>::fault_key_t check_fault_key = fault_def.get_fault_key();
