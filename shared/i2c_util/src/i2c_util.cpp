...
void i2c_util::recover_i2c(i2c_inst_t* i2c, const uint8_t sda_pin, const uint8_t scl_pin)
{
  ...
  gpio_deinit(scl_pin);
  i2c_bus_init(i2c, sda_pin, scl_pin, i2c == i2c0 ? baud_rate_bus0 : baud_rate_bus1);
}
                          ...
                          const uint8_t len)
{
  const int bytes_written = i2c_write_blocking_until(i2c, dev_addr, &reg_addr, 1, true,
                                                     delayed_by_ms(get_absolute_time(), I2C_TIMEOUT_MS));
  if (bytes_written != 1)
  {
    return false;
  }
...
uint8_t i2c_util::scan_for_devices(i2c_inst_t* i2c, const std::function<void(uint8_t)>& on_device_found)
{
