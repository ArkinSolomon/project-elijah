...
void mpu_6050::init()
{
  ...
  {
    PayloadStateManager::log_message("Could not read calibration data for MPU 6050");
  }
}
...
bool mpu_6050::check_chip_id()
{
  ...
  PayloadStateManager::log_message(std::format("Chip ID: 0x{:02X}", read_id));
...
{
  PayloadStateManager::log_message("configuring mpu6050");
...
{
  critical_section_enter_blocking(&mpu_6050_cs);
  PayloadStateManager::log_message(std::format("Calibrating MPU 6050 with {} cycles", MPU_6050_CALIBRATION_CYCLES));
  ...
  {
    PayloadStateManager::log_message("Failed to configure MPU 6050 for calibration");
    critical_section_exit(&mpu_6050_cs);
    return;
  }
  ...
  if (!configure_success)
  {
    PayloadStateManager::log_message("Failed to re-configure MPU 6050 to default settings after calibration");
    return;
  }
  ...
  {
    PayloadStateManager::log_message(std::format("Failed to write calibration data to flash, code {}", write_status));
  }
}
...
void mpu_6050::data_loop(CollectionData& collection_data)
{
  ...
  if (!device_detected)
  {
    if (!check_chip_id())
    {
      PayloadStateManager::log_message("Fault: MPU 6050, device is not detected");
    ...
    if (!device_detected)
    {
      PayloadStateManager::log_message("Fault: MPU 6050, device was detected, but failed to configure");
      set_fault(status_manager::DEVICE_MPU_6050, true);
      critical_section_exit(&mpu_6050_cs);
      return;
    }
  ...
  if (!device_detected)
  {
    PayloadStateManager::log_message("Fault: MPU 6050, failed to get values from device");
    set_fault(status_manager::DEVICE_MPU_6050, true);
    critical_section_exit(&mpu_6050_cs);
    return;
  }
