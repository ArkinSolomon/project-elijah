...
bool mount_card(FATFS* fs)
{
  ...
  if (fr != FR_OK)
  {
    PayloadStateManager::log_message(std::format("Fault: microSD card failed to mount, error {} ({})", FRESULT_str(fr),
...
bool open_file(FATFS* fs, FIL* fil, const BYTE mode)
{
  ...
  {
    PayloadStateManager::log_message(std::format("Fault: could not open {} on microSD card, error: {} ({})",
...
bool try_seek(FIL* fil, const FSIZE_t seek_pos)
{
  ...
  if (fr != FR_OK)
  {
    PayloadStateManager::log_message(std::format("Fault: failed to seek to position {}, error: {} ({})",
...
bool w25q64fv::init()
{
  PayloadStateManager::log_message("Initializing flash simulator");
  critical_section_init_with_lock_num(&flash_sim_cs, LOCK_NUM_FLASH_SIM_CS);

  if (!sd_init_driver())
  {
    PayloadStateManager::log_message("Fault: could not initialize SD card driver");
    set_fault(status_manager::MICRO_SD, true);
    return false;
  }
...
bool w25q64fv::write_sector(uint32_t sector_addr, const uint8_t* data, uint16_t data_len)
{
  ...
  {
    // PayloadStateManager::log_message(std::format("Sector address too large, can not simulate write to 0x{:06X}", sector_addr));
    // return false;
  }
...
{
  PayloadStateManager::log_message("Do not call w25q64fv::page_program when using the microSD card as a flash simulator");
  return false;
}
...
bool w25q64fv::read_data(uint32_t data_addr, uint8_t* data_buff, size_t data_len)
{
  ...
  {
    PayloadStateManager::log_message(std::format("Can not simulate read {} bytes from 0x{:06X}, would overflow flash", data_len,
                                               data_addr));
    return false;
  }
  ...
  {
    PayloadStateManager::log_message(std::format("No data to read, filling {} 0x01s", data_len));
...
bool w25q64fv::chip_erase()
{
  ...
  if (fr != FR_OK)
  {
    PayloadStateManager::log_message(std::format("Fault, failed to delete microSD card file error: {} ({})",
                                               FRESULT_str(fr), static_cast<uint8_t>(fr)));
    set_fault(status_manager::MICRO_SD, true);
    return false;
  }
