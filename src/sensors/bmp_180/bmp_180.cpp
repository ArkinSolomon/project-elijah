...
{
  uint8_t read_id;
  const bool success = i2c_util::read_ubyte(I2C_BUS, BMP_180_ADDR, _reg_defs::REG_DEVICEID, read_id);
                                  ...
                                  int32_t& pressure, double& altitude)
{
  ...
  x3 = ((x1 + x2) + 2) / 4;
  uint32_t b4 = calib_data.AC4 * static_cast<uint32_t>(x3 + 0x8000) / 0x8000;
  uint32_t b7 = (static_cast<uint32_t>(uncomp_press) - b3) * (static_cast<int32_t>(5e4) >> oss_setting);
